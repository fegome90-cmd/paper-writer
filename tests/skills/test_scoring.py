...
class TestCalculateDScore:
    ...
    def test_cohort_mid_scores(self) -> None:
        ...
        m = PaperMetrics(
            population_score=8.0,
            intervention_score=6.0,
            outcome_score=7.0,
            evidence_score=3.0,
            sample_score=1.0,
            journal_score=1.5,
...
class TestCalculateFinalScore:
    ...
    def test_custom_weights(self) -> None:
        ...
        w = ScoringWeights(A_weight=1.0, B_weight=0.0, C_weight=0.0, D_weight=0.0, E_weight=0.0)
        m = PaperMetrics(
            population_score=7.5,
            intervention_score=0.0,
            outcome_score=0.0,
            evidence_score=0.0,
            sample_score=0.0,
    ...
    def test_weights_must_sum_to_one(self) -> None:
        ...
        with pytest.raises(ValueError):
            ScoringWeights(A_weight=0.5, B_weight=0.5, C_weight=0.5, D_weight=0.0, E_weight=0.0)
        ...
        papers = [
            {
                "doi": None,
                "pmid": None,
                "title": "Voice quality after total laryngectomy: a systematic review",
            },
            {
                "doi": None,
                "pmid": None,
...
class TestVerifyCitation:
    ...
    def test_no_doi_no_pmid_returns_unverified(self) -> None:
        ...
        assert (
            "no identifier" in result.get("notes", "").lower()
            or "no doi" in result.get("notes", "").lower()
        )
    ...
    def test_doi_takes_priority_over_pmid(self) -> None:
        ...
        with (
            patch("skills.imported.literature_search.scoring._crossref_lookup") as mock_cr,
