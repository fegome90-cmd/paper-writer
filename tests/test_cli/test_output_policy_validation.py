"""Tests for output.validate_output_policy (P2.5.3, P2.8.22).
...
from cli.paper.output import OutputFormat, OutputPolicy, validate_output_policy
...
def testvalidate_output_policy_accepts_valid_combinations(
    ...
    validate_output_policy(args, fmt)


def testvalidate_output_policy_rejects_json_on_text_only() -> None:
    ...
    with pytest.raises(UserInputError, match="json is not supported"):
        validate_output_policy(args, "json")


def testvalidate_output_policy_fail_closed_on_missing_metadata() -> None:
...
def testvalidate_output_policy_skips_pipelines() -> None:
