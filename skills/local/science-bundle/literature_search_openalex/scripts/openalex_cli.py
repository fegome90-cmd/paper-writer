...
_API_CLIENT = http_client.HttpClient(BASE_URL, qps=10.0, referer_skill="literature-search-openalex")
...
def _try_download_url(url: str, output_path: str) -> bool:
    ...
    try:
        ...
        if not _is_valid_pdf(data):
            logging.warning(
                "Downloaded content from %s is not a valid PDF (missing %%PDF- header). Skipping.",
                url,
            )
            return False
...
def _try_fallback_locations(work_id: str, output_path: str, api_key: str | None) -> bool:
    ...
    landing_pages = [
        loc.get("landing_page_url") for loc in locations if loc and loc.get("landing_page_url")
    ]
    if landing_pages:
...
def handle_download_pdf(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        if not _is_valid_pdf(data):
            logging.warning("Content server returned non-PDF content (missing %%PDF- header).")
        else:
...
def handle_filter(args: argparse.Namespace) -> None:
    ...
    params = {k: getattr(args, k) for k in optional_params if getattr(args, k) is not None}
...
def handle_rate_limit(args: argparse.Namespace) -> None:
    ...
    data = fetch_with_retry(url, {"per_page": 1}, api_key=args.api_key, return_headers=True)
    if data is None:
...
def main(argv: Sequence[str]) -> None:
    ...
    parser_get.add_argument("--select", type=str, help="Limit returned fields (e.g., id,title)")
    ...
    parser_filter.add_argument("--filter", type=str, help="Filter string (e.g. is_oa:true)")
    parser_filter.add_argument("--sort", type=str, help="Sort string (e.g. cited_by_count:desc)")
    ...
    parser_filter.add_argument("--sample", type=int, help="Number of random samples to return")
    parser_filter.add_argument("--seed", type=int, help="Seed for random sampling")
    parser_filter.add_argument("--select", type=str, help="Limit returned fields (e.g., id,title)")
