...
def fetch_article_abstracts(
    ...
    params = _env_params() | id_p | {"db": "pubmed", "rettype": "abstract", "retmode": "xml"}
...
def find_linked_biological_data(
    ...
    if not linksets:
        print(
            f"Got empty linksets for {source_pmid}, {target_database}, {linkname}, {dbfrom}",
            file=sys.stderr,
        )
        return []
    ...
    if not linksetdbs:
        print(
            f"Got empty linksetdbs for {source_pmid}, {target_database}, {linkname}, {dbfrom}",
            file=sys.stderr,
        )
        return []
