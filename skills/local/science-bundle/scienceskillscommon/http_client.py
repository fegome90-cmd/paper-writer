...
SKILL_REFERER_TEMPLATE: str = "https://github.com/google-deepmind/science-skills/tree/main/{skill}"

# Regex for parsing X-Throttling-Control status entries.
# Matches e.g. "Request Count status: Red (82%)".
_THROTTLE_STATUS_RE = re.compile(r"(\w[\w ]*?) status:\s*(Green|Yellow|Red|Black)\s*\((\d+)%\)")
...
class HttpResponse:
    ...
    def __repr__(self) -> str:
        return f"HttpResponse(status={self.status_code}, url={self.url!r}, size={len(self.data)})"
...
class HttpClient:
    ...
    def __init__(
        ...
        *,
        ...
        self.jitter = jitter
        self.user_agent = user_agent or os.environ.get("SCIENCE_SKILLS_USER_AGENT", "")
    ...
    def _build_request(
        ...
        if self._referer_skill:
            merged_headers["Referer"] = SKILL_REFERER_TEMPLATE.format(skill=self._referer_skill)
    ...
    def fetch(
        ...
        *,
        ...
        """
        with self._open_stream(url, method, headers, data, json_body, timeout, max_retries) as resp:
    ...
    def _open_stream(
        ...
        for attempt in range(effective_retries + 1):
            ...
            try:
                ...
                if status in self.retryable_status_codes and attempt < effective_retries:
                    next_min_sleep = self._compute_backoff(attempt, retry_after)
                    logging.info(
                        "HttpClient[%s]: HTTP %d from %s — retrying in ≥%.1fs (attempt %d/%d)",
                        self.hostname,
                        status,
                        url,
                        next_min_sleep,
                ...
                if attempt < effective_retries:
                    ...
                    logging.info(
                        "HttpClient[%s]: Network error (%s) — retrying in ≥%.1fs (attempt %d/%d)",
                        self.hostname,
                        exc,
                        next_min_sleep,
                        attempt + 1,
                    ...
                    last_exc = exc
                    continue
                raise HttpError(f"Network error fetching {url}: {exc}", url=url) from exc
