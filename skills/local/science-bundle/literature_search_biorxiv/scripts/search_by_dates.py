...
def search_biorxiv(args):
    ...
    base_url = f"https://api.biorxiv.org/details/{args.server}/{args.start_date}/{args.end_date}"
    cursor = 0
    all_results = []
    total_fetched = 0
    ...
    print(
        f"Fetching metadata for {args.server} from {args.start_date} to {args.end_date}...",
        file=sys.stderr,
    )
    if args.category:
