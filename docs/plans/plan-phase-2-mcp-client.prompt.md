...
**Note on function names**: `audit_prose`, `audit_claims`, `gate_method` are NEW public function names created in PR #1 as the resolution layer's stable entry points in `harness/services/audit/{prose,claims,method_gate}.py`. The current codebase uses different names (`run_prose_audit`, `build_claims_report`, `MethodGateValidator.validate`); PR #1 introduces the new names without removing the old ones (deprecated aliases).
