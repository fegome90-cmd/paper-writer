...
1. Move all inline Fase 0 logic (`_cmd_audit_prose`, `_cmd_audit_claims`, `_cmd_gate_method`) from `cli/paper/main.py` into `harness/services/audit/` so the CLI entrypoint contains no inline validator calls (Fase 0 command bodies moved to `harness/services/audit/`; entrypoint retains argument parsing, command-to-`OrchestratorRequest` mapping, and `_print_summary` per §19 / per current file: `main` function at line 174 — subparser setup 175-330, command-to-`OrchestratorRequest` mapping at lines 353-403, dispatch at 405-413, `_print_summary` call at 415-417; `_print_summary` function at lines 420-453; `__main__` block at lines 456-457).
...
5. Implement a fail-closed default with an explicit opt-in (`fallback: local`) for resolvers where a local implementation exists. MCP-only tools (tools without a local path — in v1: `paper_evidence_map`, `paper_reviewer2`) ALWAYS fail-closed (schema-level enforcement: `fallback` key rejected at config load for any tool whose local resolver does not exist).
...
3. **Not** a Fase 0/1 expansion. No new validators, no new rule families, no new CLI commands beyond the resolution mechanism. P1/P2 tools listed in `mcp-tools-candidates.md` are **out of v1** scope. The "MCP-only" classification is priority-agnostic — it is defined by the absence of a local path, not by P-tier. The v1 MCP-only P0 set is `paper_evidence_map`, `paper_reviewer2`; the schema rule that rejects `fallback: local` is the single source of truth (see §7).
...
- **Decision**: `MCPAdapter` is a `ToolWrapper` concrete subclass. No new port. **The orchestrator does not change** (no new constructor arg, no new field; the orchestrator's resolution layer reads from `wrappers`, not from `mcp_clients`). The `mcp_clients: MappingProxyType[str, MCPAdapter]` slot on `OrchestratorDependencies` mirrors the `skill_adapters` precedent — present for builder-level testability and decoupling, NOT for Orchestrator consumption.
...
class ToolWrapper(ABC):
    ...
    class MCPAdapter(ToolWrapper):
        def __init__(self, transport: MCPTransport, timeouts: Timeouts, adapter_id: str) -> None: ...
        def run(self, artifacts: dict[str, Any], context: dict[str, Any]) -> ValidatorResult: ...
        def is_available(self) -> bool: ...
        @property
        def name(self) -> str: ...
...
The resolution layer (PR #3) reads from `wrappers`. The `mcp_clients` field on `OrchestratorDependencies` mirrors the `skill_adapters` precedent — present for builder-level testability and decoupling, not for Orchestrator consumption.
...
**Dual-dict relationship**: The `OrchestratorDependencies.mcp_clients` slot holds MCPAdapter instances keyed by `adapter_id` (e.g., `'robin'`, `'era'`). The `OrchestratorDependencies.wrappers` slot holds the SAME MCPAdapter instances, re-keyed by orchestrator `command_name` (e.g., `'paper_claim_audit'`, `'paper_evidence_map'`, `'paper_reviewer2'`). The builder populates both: it reads `.paper-writer/mcp.yaml`, constructs one `MCPAdapter` per `adapter_id` entry, then registers each adapter under all `command_name`s it serves (per the `resolvers:` section of the config). The two dicts share the same instance references — they are not separate copies. The `mcp_clients` slot is for builder-level testability and CLI introspection (`paper doctor` reads it to count adapters); the `wrappers` slot is what `_run_wrapper_gate` actually dispatches against. **Lifecycle**: in v1, the `MCPAdapter` instance is constructed at builder time (PR #2); the underlying transport is spawned lazily on first `run()` call and torn down on process exit. **State after PR #2 lands with no MCP adapters declared in `.paper-writer/mcp.yaml`**: `mcp_clients` is an empty `MappingProxyType({})`; `wrappers` contains only the 7 existing entries plus the 3 new audit wrappers added in PR #1.
...
**Constructor signature and timeout mapping**: The `MCPAdapter` constructor signature is `__init__(self, transport: MCPTransport, timeouts: Timeouts, adapter_id: str)` where `Timeouts` is a frozen dataclass `{connect_ms: int, request_ms: int}` defined in `harness/services/mcp/types.py` (a shared types module introduced in PR #2 alongside `responses.py`; future request/response dataclasses can also live here). The config schema in §7 (`timeouts: { connect_ms, request_ms }`) maps directly: `Timeouts(connect_ms=config["timeouts"]["connect_ms"], request_ms=config["timeouts"]["request_ms"])`. The transport consumes `connect_ms`; the per-call timeout consumes `request_ms`.

**Transport contract**: `MCPTransport` is an abstract base class defined in `harness/services/mcp/transports/__init__.py`; `transports/stdio.py` is the v1 concrete implementation (subprocess JSON-RPC over stdio). The transport layer is responsible for spawn, lifecycle, and error trapping — it MUST catch all `OSError` (including `FileNotFoundError` for missing subprocess commands, `PermissionError`, `TimeoutError` on `communicate()`) and re-raise as one of the 5 declared error classes. The only exceptions that may propagate from `MCPTransport` to `MCPAdapter` are the 5 declared error classes; `OSError` MUST NOT reach the adapter boundary.
...
resolvers:
  ...
  paper_reviewer2:
    adapter: robin                # MCP-only; fallback key rejected by schema
  paper_method_gate:
    adapter: era                  # default local; fallback: local allowed because MethodGateValidator exists (PR #1 refactor)
    fallback: local              # opt-in; absent = fail-closed
    # NB: local path is `harness.services.audit.method_gate.gate_method` (PR #1)
```
...
3. **Adapter configured and unreachable, `fallback: local` set, and a local wrapper exists** → use the local wrapper and emit a `warning: fallback triggered for <tool>: mcp <adapter> unavailable, using local`. The step entry records the path switch. **"Local wrapper exists" source of truth**: the schema's "local-exists" list (defined in the paragraph below — derived from §9 P0 table and §19, not from the runtime `wrappers` dict). The runtime `wrappers` dict is the dispatch surface, but rule 3's eligibility check MUST consult the schema list, not the dict, so that tools added in future PRs are not silently ineligible for fallback because their wrapper hasn't been wired yet.
...
| Tool | Command name (passed to `OrchestratorRequest.command`) | Service function (`module.path.func`) | MCP adapter slot | Fallback allowed |
|------|-------|------|------|------|
| `paper_claim_audit` | `audit_claims` (new) — added in PR #1 (registered in `Orchestrator._validate_preconditions` / `_run_gate_verification` / `_get_next_stage`); MCP resolution path wired in PR #5. | `harness.services.audit.claims.audit_claims` (PR #1) | `robin.paper_claim_audit` | yes (local exists) |
| `paper_evidence_map` | `map_evidence` (new) — added in PR #3 (registered in `Orchestrator._validate_preconditions` / `_run_gate_verification` / `_get_next_stage`); local resolver stays "none in v1" — only the MCP path is exercisable. | none in v1 | `robin.paper_evidence_map` | no (MCP-only) |
| `paper_reviewer2` | `reviewer2_run` (new) — added in PR #3 (registered in `Orchestrator._validate_preconditions` / `_run_gate_verification` / `_get_next_stage`); local resolver stays "none in v1" — only the MCP path is exercisable. | none in v1 | `robin.paper_reviewer2` | no (MCP-only) |
| `paper_method_gate` | `gate_method` — added in PR #1 (registered in `Orchestrator._validate_preconditions` / `_run_gate_verification` / `_get_next_stage`); MCP resolution path wired in PR #5. | `harness.services.audit.method_gate.gate_method` (PR #1, calls `MethodGateValidator`) | `era.paper_method_gate` | yes (local exists) |

**Note on function names**: The current codebase exposes `ProseValidator.validate`, `ClaimsValidator.validate`, `MethodGateValidator.validate` (one entry per validator). PR #1 introduces `audit_prose`, `audit_claims`, `gate_method` as new `ToolWrapper` SUBCLASSES (not just functions) in `harness/services/audit/wrappers.py` — `ProseAuditWrapper(ToolWrapper)`, `ClaimsAuditWrapper(ToolWrapper)`, `MethodGateWrapper(ToolWrapper)` — that wrap the existing Fase 0 validators and return `ValidatorResult` (so the orchestrator's `_run_wrapper_gate` can dispatch them via the existing `wrappers` dict). The current validator entry points stay where they are; no validator logic is duplicated. `harness/services/audit/{prose,claims,method_gate}.py` continue to hold the validator-invoking service functions used by the wrapper classes.
...
**Credentials**: env vars only in v1. The schema for the `env:` section in `.paper-writer/mcp.yaml` is `Mapping[str, SecretReference]` where `SecretReference = str` matching the pattern `^[A-Z][A-Z0-9_]*$` (e.g., `OPENAI_API_KEY`). `SecretReference` is defined in `harness/services/mcp/schema.py` and re-exported from `harness/services/mcp/sanitization.py`. At adapter construction time, the env var is resolved against `os.environ`. The **resolved value never appears** in `OrchestratorResult.steps`, in `state.yaml`, in the artifact directory, or in any log line. A `SecretDetector` utility in `harness/services/mcp/sanitization.py` enforces this at two checkpoints in v1: step-emit and artifact-write. Log-emit wiring is a follow-up (not in v1).
...
```python
{
    ...
    "sanitized_request": MCPRequestPayload,  # Pydantic model in harness/services/mcp/responses.py (same file as response schema per Q6); JSON-RPC payload with secrets redacted
...
- `path: "local_via_fallback"` — adapter was configured and unreachable; `fallback: local` was set, and the local wrapper was used. The same step entry also carries `status: "succeeded"` and a sibling `warning` entry on the result. **Field preservation**: when `path == "local_via_fallback"`, the step entry PRESERVES the failing adapter's `adapter_id` and `transport` (and `error_class`) so log scrapers can trace the original MCP attempt; the `path` field alone signals the resolution outcome. CI rules can grep `path == "local_via_fallback"` to count fallback firings without losing the adapter context.
...
- [ ] Create `docs/SECURITY.md` with the v1 MCP credential storage policy (env-var-only, no keychain in v1, precedence: env > config) — deliverable as part of PR #2.
...
|----|-------|---------------|------|
| **#1** | Refactor Fase 0: move `_cmd_audit_prose`, `_cmd_audit_claims`, `_cmd_gate_method` from `cli/paper/main.py` to `harness/services/audit/`. The dispatch in `cli/paper/main.py:333-336` (the `func = getattr(args, "func", None)` direct-call path) is REPLACED with an `Orchestrator.execute` call for Fase 0 commands. `cli/paper/main.py` becomes pure dispatch (no inline validator calls). **PR #1 wires 3 commands (`audit_prose`, `audit_claims`, `gate_method`)** into the orchestrator: (a) add `audit_prose`, `audit_claims`, `gate_method` to `Orchestrator._validate_preconditions` precondition map (so they pass precondition check); (b) add dispatch cases in `Orchestrator._run_gate_verification` for the three commands; (c) add stage mappings in `Orchestrator._get_next_stage` — all three map to `"unknown"` in v1 (terminal audit/gate commands, no stage progression); (d) refactor moved service functions (`harness/services/audit/{prose,claims,method_gate}.py`) to return result objects — NO `sys.exit(1)` inside the service layer; the CLI/orchestrator boundary performs the exit code mapping based on result status. Note: `harness/services/audit/{prose,claims,method_gate}.py` are thin wrappers that import and call the existing Fase 0 validators in `validators/`; no validator logic is duplicated. The public entry points are `audit_prose`, `audit_claims`, `gate_method` (functions that return result dataclasses — `sys.exit` is NOT called in the service layer). | `cli/paper/main.py` (`_cmd_audit_prose`, `_cmd_audit_claims`, `_cmd_gate_method` moved out, AND extend the command-to-`OrchestratorRequest` mapping at lines 353-403: add `audit_prose` → `audit_prose`, `audit_claims` → `audit_claims`, `gate method` → `gate_method` cases under the `cmd_name == "audit"` / `cmd_name == "gate"` branches; current `main()` only maps `audit reporting` at lines 384-385), `harness/services/orchestrator.py` (add commands to precondition map, dispatch cases, stage mappings; `_get_next_stage` maps all three to `"unknown"`), `harness/services/orchestrator_builder.py` (register the 3 new audit wrappers under keys `audit_prose`, `audit_claims`, `gate_method` in the `wrappers` dict; existing 7 entries unchanged), `harness/adapters/filesystem_action_runner.py` (PR #1 does NOT modify this file; wiring is handled via `Orchestrator.execute` and `OrchestratorDependencies.action_runner`), NEW: `harness/services/audit/{prose,claims,method_gate}.py`, NEW: `harness/services/audit/wrappers.py` (separate `ToolWrapper` subclasses: `ProseAuditWrapper`, `ClaimsAuditWrapper`, `MethodGateWrapper` — wrap the Fase 0 validators and return `ValidatorResult`; chosen over co-locating inside `{prose,claims,method_gate}.py` for separation between validator logic and orchestrator integration), NEW: `harness/services/audit/__init__.py`, NEW: `tests/integration/` directory created in PR #1, NEW: `tests/integration/test_fase0_routing.py`. | Fase 0 commands route through `Orchestrator.execute`; existing Fase 0 tests pass with their result content unchanged (functional regression: `pytest tests/cli/test_cli_phase0_regression.py`); `_validate_preconditions` and `_run_gate_verification` accept the 3 new command names; `cli/paper/main.py` no longer contains `_cmd_audit_prose`, `_cmd_audit_claims`, or `_cmd_gate_method` (verify via `grep -n "_cmd_audit" cli/paper/main.py` returns no matches); service layer does NOT call `sys.exit` (verify via `grep -n "sys.exit" harness/services/audit/` returns no matches; exit code mapping lives at the orchestrator boundary in `_print_summary`). **Argument plumbing**: `gate_method` accepts `--study-type` → `args["study_type"]`, `--checklist` → `args["checklist"]`, `--na` → `args["na"]` (repeatable list) — these are forwarded through `OrchestratorRequest.args`. `audit_prose` and `audit_claims` plumb `file` (positional) and `--whitelist` → `args["whitelist"]` (repeatable list). PR #1 verification: each CLI arg round-trips to the service function via `OrchestratorRequest.args`.
| **#2** | Adapter contract: define `MCPAdapter(ToolWrapper)` in `harness/services/mcp/adapter.py`, add `mcp_clients: MappingProxyType[str, MCPAdapter]` slot to `OrchestratorDependencies`, register a stub transport (`harness/services/mcp/transports/stdio.py`). PR #2 also adds `paper doctor` MCP adapter count reporting (configured vs reachable, defaults to `0 configured, 0 reachable` in v1). Creates `tests/integration/` directory (if not already created in PR #1) and `tests/integration/test_mcp_adapter.py` (stub transport). | NEW: `harness/services/mcp/{adapter.py,transports/__init__.py,transports/stdio.py,errors.py,responses.py}`, NEW: `harness/services/mcp/types.py` (shared types module: `Timeouts` frozen dataclass + future request/response dataclasses), NEW: `docs/SECURITY.md` (v1 MCP credential storage policy), `.gitignore` (appends `.paper-writer/mcp.local.yaml` and `*.mcp.secret` to prevent accidental commit of local configs and credential material), modify `harness/services/orchestrator_builder.py` (add `mcp_clients` field + builder arg), modify `harness/services/orchestrator.py` (no semantic change; just thread the new field). **Q1 LOCKED invariant**: `Orchestrator.__init__` signature is unchanged — PR #2 only modifies the `OrchestratorDependencies` builder and the `OrchestratorDependencies` class; `Orchestrator` itself does not change. | `tests/harness/test_mcp_adapter.py` ≥ 80% coverage. `_run_wrapper_gate` behavior unchanged. Existing builder tests still pass. `paper doctor` reports adapter count. Stub MCP transport ready and used in `tests/integration/test_mcp_adapter.py`; CI does not require network for adapter tests. `docs/SECURITY.md` exists and contains the v1 MCP credential storage policy. **Contract assertion**: `harness/services/mcp/errors.py` defines all custom errors as `RuntimeError` subclasses; verified by `pytest tests/harness/test_mcp_errors.py::test_all_errors_inherit_from_runtime_error`. |
...
| **#4** | Fallback semantics: implement the four-rule fallback logic, integrate step entries, wire `SecretDetector` into step-emit and artifact-write paths, add the "FALLBACK TRIGGERED" suffix to the one-line summary. PR #4 modifies `harness/adapters/filesystem_action_runner.py` (sanitize `write_command_log` payload via `SecretDetector` and the new `emit_manifest` sanitization) and ensures `MCPAdapter` calls `SecretDetector.sanitize` on its own artifact-write path before persisting. Creates `tests/security/` directory and `tests/security/test_secret_detector.py`. | modify `harness/services/orchestrator.py` (fallback dispatch + step emission), NEW: `harness/services/mcp/sanitization.py`, NEW: `tests/security/` directory, NEW: `tests/security/test_secret_detector.py`, modify `cli/paper/main.py` (`_print_summary`), modify `harness/adapters/filesystem_action_runner.py` (sanitize on emit). | `tests/integration/test_fallback.py` passes. MCP-down → blocker for MCP-only tools; MCP-down → local + warning for `fallback: local` tools. `pytest tests/security/test_secret_detector.py` (or equivalent) passes. |
...
- [ ] No regression: all Fase 0/1 tests pass; `Orchestrator.__init__` is **unchanged** (Q1 LOCKED — no new args, no new fields, no constructor modification). `OrchestratorDependencies` gains an optional `mcp_clients: MappingProxyType[str, MCPAdapter]` slot that defaults to empty `MappingProxyType({})` (backward-compat: existing constructors and existing serialization paths continue to work; the slot is parallel to `skill_adapters` — for builder-level testability, not for Orchestrator consumption).
...
| Credential leakage via logs / `state.yaml` / artifacts | Medium | Critical | `SecretDetector` utility at two checkpoints in v1 (step-emit, artifact-write; log-emit is a follow-up); CI linter that fails when `state.yaml` or any emitted artifact contains a high-entropy string; env-var-only storage in v1; `.gitignore` rules for `.paper-writer/mcp.local.yaml`. |
...
**Out-of-scope for v1 (PR #5 follow-up)** — these items are deliberately NOT in §14 success criteria:
...
- The `audit_claims` (and any other) orchestrator command(s) needed by `paper review` are defined in the PR #5 follow-up spec. v1 plumbing supports them without yet exercising them.
...
- **2026-06-02 — Q1 LOCKED**: `MCPAdapter` is a `ToolWrapper` concrete subclass. No new port. **The orchestrator does not change** (no new constructor arg, no new field; the orchestrator's resolution layer reads from `wrappers`, not from `mcp_clients`). The `mcp_clients: MappingProxyType[str, MCPAdapter]` slot on `OrchestratorDependencies` mirrors the `skill_adapters` precedent — present for builder-level testability and decoupling, NOT for Orchestrator consumption. Rationale: keeps the orchestrator simple, leverages the existing `_run_wrapper_gate` fail-closed path, and avoids the cost of a sibling port.
...
6. **Implementation notes for PR #1–#4 (non-blocking, captured 2026-06-02 from trifecta cross-reference audit)**: the following items are clarifications the implementer should be aware of; they do NOT change the architecture but reduce ambiguity at PR time:
   - **PR #1 — CLI-to-OrchestratorRequest mapping**: the current `main()` at `cli/paper/main.py:382-385` only maps `audit reporting` to `audit_reporting`. PR #1 must add mapping cases for `audit_prose` → `audit_prose`, `audit_claims` → `audit_claims`, and `gate method` → `gate_method` in the `cmd_name == "audit"` / `cmd_name == "gate"` branches, otherwise the orchestrator is never invoked for these commands. (M1: §13 PR #1 row.)
   - **PR #1 — `gate_method` argument plumbing**: `gate_method` accepts `--study-type`, `--checklist`, and `--na` (main.py:253-275). PR #1 must forward these through `OrchestratorRequest.args` (e.g., `args={"study_type": ..., "checklist": ..., "na": [...]}`) to the `gate_method` service. `audit_prose` and `audit_claims` only need `file` and `--whitelist` forwarded.
   - **PR #1 — `_get_next_stage` mappings**: the three Fase 0 commands map to stage `"unknown"` in v1 (no stage-progression contract for them yet; they are terminal audit/gate commands). PR #1 hard-codes `audit_prose`/`audit_claims`/`gate_method` → `"unknown"` in the stage map.
   - **PR #2 — Q1/PR #2 wording reconciliation**: §17 line 397 (Q1 LOCKED) says "the orchestrator does not change" while §13 PR #2 (line 335) says "modify `harness/services/orchestrator.py`". The intent is: PR #2 modifies `harness/services/orchestrator_builder.py` (adds `mcp_clients` field + builder arg) and `OrchestratorDependencies` class; `harness/services/orchestrator.py`'s `Orchestrator.__init__` is unchanged (verified by verification step #13 at the end of §20). Future spec revisions should treat this distinction as: "the orchestrator constructor is unchanged" (Q1 invariant) vs "the dependencies builder accepts a new optional slot" (PR #2 scope).
   - **PR #2 — `mcp_clients` default factory**: `OrchestratorDependencies` is a frozen dataclass; the `mcp_clients` field MUST use `field(default_factory=lambda: types.MappingProxyType({}))` to avoid a shared mutable default instance. (Implementation detail, not a spec change.)
   - **PR #4 — `local_via_fallback` field preservation**: the step entry's `adapter_id`/`transport`/`error_class` are preserved when `path == "local_via_fallback"` (see §11 path field semantics); only `path` changes.
...
- NEW: `harness/services/mcp/types.py` — shared types module: `Timeouts` frozen dataclass (PR #2). Future request/response dataclasses can also live here.
...
- NEW: `harness/services/audit/__init__.py` + `prose.py` + `claims.py` + `method_gate.py` + `wrappers.py` — PR #1 refactor target. `wrappers.py` defines the 3 `ToolWrapper` subclasses (`ProseAuditWrapper`, `ClaimsAuditWrapper`, `MethodGateWrapper`) that wrap the Fase 0 validators and return `ValidatorResult`.
...
6. `python -m paper audit claims --output json <file>` (or `python -m paper audit prose <file>` / `python -m paper gate method <file>`) — Fase 0 commands route through `Orchestrator.execute` and produce the same output as pre-PR-1 (regression test in `tests/cli/test_cli_phase0_regression.py`). **Note**: the v1 plumbing supports `--mcp-resolver=...=stub` as a no-op (the CLI parser accepts the flag for forward-compat with PR #3; the resolution layer is not wired in v1 plumbing — it ships in PR #3).
...
13. `python -c "from harness.services.orchestrator import Orchestrator; import inspect; assert list(inspect.signature(Orchestrator.__init__).parameters) == ['self','repo_path','state_manager','checker','action_runner','wrappers']"` — `Orchestrator.__init__` signature unchanged (Q1 LOCKED direct check).
