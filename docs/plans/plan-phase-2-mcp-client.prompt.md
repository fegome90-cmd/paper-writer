...
| `paper_evidence_map` | `map_evidence` (new) — added in PR #1 (registered in `Orchestrator._validate_preconditions` / `_run_gate_verification` / `_get_next_stage`); MCP resolution path wired in PR #5. Local resolver stays "none in v1" — only the MCP path is exercisable. | none in v1 | `robin.paper_evidence_map` | no (MCP-only) |
| `paper_reviewer2` | `reviewer2_run` (new) — added in PR #1 (registered in `Orchestrator._validate_preconditions` / `_run_gate_verification` / `_get_next_stage`); MCP resolution path wired in PR #5. Local resolver stays "none in v1" — only the MCP path is exercisable. | none in v1 | `robin.paper_reviewer2` | no (MCP-only) |
